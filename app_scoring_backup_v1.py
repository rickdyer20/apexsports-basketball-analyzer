...
• **Proper Release Mechanics** - Your shooting form shows solid fundamentals in this attempt, with good positioning and timing
• **Good Shooting Plane** - You're keeping the ball on a straight path to the basket, eliminating wasted energy and improving accuracy

**Your Competitive Advantage:**
With form this solid, you're already performing better than 85-90% of players at most levels. This shot demonstrates excellent technique that, when repeated consistently, gives you a massive advantage in game situations where other players' shots fall apart under pressure. Coaches love players they can count on for reliable scoring.

**Maintaining Excellence:**
Even elite-level shots need regular form analysis to stay sharp. Continue using our program to monitor your mechanics across multiple shots and catch any small adjustments before they become habits. Your dedication to improvement is what will help you maintain this high level and potentially take you even higher!"""
        ...
        # Determine overall performance level and tone - RECALIBRATED SCORING
        if overall_score >= 0.80:
            performance_level = "ELITE"
            improvement_potential = "fine-tuning for championship performance"
            ...
            improvement_potential = "strong foundation with key areas to develop"
            timeline = "6-8 weeks of dedicated training"
            percentage_improvement = "25-35%"
        else:
            performance_level = "DEVELOPING"
            improvement_potential = "tremendous upside for dramatic improvement"
            timeline = "8-12 weeks of fundamental training"
            percentage_improvement = "35-50%"
...
"""
        ...
        if 'release_height' not in flaws:
            strengths.append("**Solid Release Mechanics** - Your form shows good fundamentals in this attempt")
...
Purpose: {plan.get('program_usage', {}).get('purpose', 'Track progress and improvements')}
        ...
        try:
            ...
            if current_phase == "preparation":
                ...
                if balance_deviation > 0.015:  # More sensitive threshold
                    # Recalibrated severity - much less harsh
                    severity = min(balance_deviation * 100, 60)  # Max 60% severity for balance issues
                ...
                if stance_width < 0.09 or stance_width > 0.20:  # Tighter optimal range
                    # Recalibrated severity - more reasonable stance penalties
                    severity = 35 if stance_width < 0.09 else min((stance_width - 0.20) * 100, 40)
                ...
                if avg_knee_angle < 150 or avg_knee_angle > 170:  # Preparation should have slight bend
                    ...
                    # Recalibrated severity - much less harsh for knee bend deviations
                    severity = (deviation / 20) * 50  # Max 50% severity instead of 100%
                ...
                if shoulder_tilt > 0.012:  # Very sensitive for preparation phase
                    # Recalibrated severity - much more reasonable
                    severity = min(shoulder_tilt * 200, 45)  # Max 45% severity for shoulder tilt
            ...
            # SHOOTING PHASE - Mechanical Execution (Power Generation)
                ...
                if not self.release_detected and not too_late_in_motion:  # Early shooting motion only
                    ...
                    if hand_elevated and arm_partially_extended:  # Check when shooting motion begins
                        ...
                        if hand_alignment > 0.08:  # More sensitive threshold
                            # Recalibrated - hand alignment severity
                            severity = min(hand_alignment * 150, 55)  # Reduced from 400, max 55%
                ...
                if avg_knee_angle < 140 or avg_knee_angle > 160:  # Should show power extension
                    ...
                    # Recalibrated - wrist position severity
                    severity = (deviation / 15) * 55  # Max 55% severity instead of 100%
                ...
                if shooting_plane_deviation > 0.04:  # More sensitive
                    # Recalibrated - shooting plane severity (was way too harsh)
                    severity = min(shooting_plane_deviation * 200, 65)  # Reduced from 800, max 65%
            ...
            # RELEASE PHASE - Ball Departure and Follow-through (Accuracy Critical)
                ...
                if release_height < 0.05:  # Higher threshold for optimal arc
                    # Recalibrated - release height severity
                    severity = min((0.12 - release_height) * 150, 50)  # Reduced from 400, max 50%
                ...
                if follow_through < 50 or follow_through > 75:  # Tighter range
                    ...
                    # Recalibrated - follow through angle severity  
                    severity = (deviation / 18) * 60  # Max 60% severity instead of 100%
                ...
                if extension_angle < 160:  # Should be near full extension
                    # Recalibrated - arm extension severity
                    severity = min((170 - extension_angle) * 1.5, 55)  # Reduced from *3, max 55%
            ...
            # FOLLOW_THROUGH PHASE - Post-Release Mechanics
                ...
                if balance_deviation > 0.06:  # More reasonable balance threshold - allow some natural movement
                    # Recalibrated - balance during shot severity
                    severity = min(balance_deviation * 120, 60)  # Reduced from 300, max 60%
                ...
                if hasattr(self, 'previous_follow_through_wrist_angle'):
                    ...
                    if wrist_snap_speed < 8 or wrist_snap_speed > 25:  # Optimal 8-25° per frame
                        ...
                        # Recalibrated - wrist snap speed severity
                        severity = (speed_deviation / 10) * 45  # Max 45% severity for timing issues
    ...
    def analyze_detailed_flaws(self, landmarks) -> Dict[str, Any]:
        ...
        # Calculate overall score based on detected flaws - RECALIBRATED WEIGHTS
        priority_weights = {1: 0.15, 2: 0.12, 3: 0.09, 4: 0.06, 5: 0.03, 6: 0.03}  # Reduced by ~40%
        total_impact = 0
        ...
        for flaw_data in flaws.values():
            ...
            # Further reduce impact - be more forgiving to good shooters
            total_impact += (severity / 100) * weight * 0.3  # Further reduced impact
        
        # Recalibrated scoring system - more responsive to actual performance
        # Start from neutral baseline and adjust based on detected flaws
        baseline_score = 0.75  # Start from 75% neutral baseline
        overall_score = baseline_score - total_impact
        
        # Ensure realistic scoring range - allow for both poor and excellent performance
        overall_score = max(0.25, min(0.95, overall_score))  # Range: 25% to 95%
        ...
        try:
            ...
            # Calculate overall form score with RECALIBRATED research-backed weighting
            priority_weights = {
                1: 0.15,  # Balance/Stance - reduced from 0.25
                2: 0.12,  # Elbow alignment - reduced from 0.20  
                3: 0.09,  # Knee bend - reduced from 0.15
                4: 0.07,  # Release point - reduced from 0.12
                5: 0.06,  # Follow-through - reduced from 0.10
                6: 0.05,  # Shooting arc - reduced from 0.08
                7: 0.03,  # Hand position - reduced from 0.05
                8: 0.02,  # Secondary focus - reduced from 0.03
                9: 0.01   # Advanced development - reduced from 0.02
            }
            ...
            for flaw_key, flaw_data in flaws.items():
                ...
                severity = flaw_data.get('severity', 0)
                weight = priority_weights.get(priority, 0.01)
                # Further reduce severity impact - be much more forgiving
                total_impact += (severity / 100) * weight * 0.4  # Further reduced to 40%
            
            # Recalibrated scoring - more responsive to actual performance
            baseline_score = 0.70  # Start from 70% neutral baseline  
            overall_score = baseline_score - total_impact
            # Allow for full range of scoring - from poor to excellent
            overall_score = max(0.20, min(0.95, overall_score))  # Range: 20% to 95%
            ...
            if hasattr(st.session_state, 'analyzer') and st.session_state.analyzer.shot_sequence:
                ...
                with st.expander("🏀 View Detailed AI Metrics & Technical Data", expanded=False):
                    ...
                    with metric_container:
                        ...
                        st.markdown("##### 📈 Form Score Analysis")
                        st.markdown("""
                        ...
                        **💡 Improvement Tips:**
                        ...
                        st.markdown("##### 📈 Release Angle Analysis")
                        st.markdown("""
                        ...
                        **📊 Optimal Ranges:**
                        ...
                        st.markdown("##### 📈 Launch Height Analysis")
                        st.markdown("""
                        ...
                        **🎯 Improvement Tips:**
                        ...
                        st.markdown("##### 📈 Elbow Alignment Analysis")
                        st.markdown("""
                ...
                if hasattr(st.session_state.analyzer, 'last_analysis_result'):
                    ...
                    if last_result and 'detailed_analysis' in last_result:
                        ...
                        if flaws:
                            ...
                            st.markdown("##### 📈 Understanding Your Research Assessment Score")
                            st.markdown(f"""
                            ...
                            **📊 Recalibrated Score Ranges:**
                            - 🟢 **80-95%**: Elite shooter - NBA/Professional level technique
                            - 🟡 **60-79%**: College level - Advanced technique and consistency  
                            - 🟠 **40-59%**: High school varsity - Good fundamentals, needs refinement
                            - 🟡 **25-39%**: Developing shooter - Focus on fundamental mechanics
                            - � **20-24%**: Beginner shooter - Basic form development needed
                    ...
                    if overall_score < 0.6:
                        ...
                        • Focus on balance and proper release mechanics
                        </div>
                        """, unsafe_allow_html=True)
